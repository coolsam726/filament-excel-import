...
trait HasImportAction
{
    ...
    protected ?Closure $createRecordUsing = null;
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (): string => __('coolsam-excel::actions.import'));
        ...
        $this->action(function (ComponentContainer $form): void {
            ...
            $this->process(function (array $data) use ($model) {
                ...
                FilamentImport::make(spreadsheetFilePath: $data['file'])
                    ...
                    ->handleRecordCreation($this->createRecordUsing)
                    ->execute();
            });
        });
    }
    ...
    public function setInitialForm(): void
    {
        $this->form([
            ...
            Toggle::make('skipHeader')
                ->default(true)
                ->label(__('coolsam-excel::actions.skip_header')),
        ]);
    }
    ...
    public function fields(array $fields, int $columns = 1): static
    {
        ...
        $this->form(
            array_merge(
                $this->form,
                [
                    Fieldset::make(__('coolsam-excel::actions.match_to_column'))
                        ->schema($fields)
                        ->columns($columns)
    ...
    private function getFields(ImportField | Field $field): Field
    {
        ...
        return Select::make($field->getName())
            ...
            ->options(options: function (Get $get, Set $set) use ($field) {
                ...
                if (count($options) == 0) {
                    $options = collect($this->toCollection($filePath)->first()?->first())->filter(fn ($value) => $value != null)->map('trim')->toArray();
                }
    ...
    public function createRecordUsing(Closure $closure): static
    {
        $this->createRecordUsing = $closure;
